...
int CountNonZero(const std::vector<double> &column, double epsilon) {
  return std::count_if(column.begin(), column.end(), [epsilon](double val) { return std::abs(val) > epsilon; });
}
...
SparseMatrixCCS KotelnikovaATaskSTL::MultiplyMatrices(const SparseMatrixCCS &a, const SparseMatrixCCS &b) {
  ...
  std::vector<int> column_indices(b.cols);
  std::iota(column_indices.begin(), column_indices.end(), 0);

  std::for_each(std::execution::par, column_indices.begin(), column_indices.end(), [&](int j) {
